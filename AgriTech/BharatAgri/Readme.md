...
```mermaid
flowchart TD
    A[Step 1: Subscription Sign-up<br/>7-day free trial] 
    --> B[Step 2: Baseline Profiling<br/>Crop, soil, irrigation data]
    B --> C[Step 3: Weekly AI-Powered Plans<br/>Personalized action items]
    C --> D[Step 4: Human Agronomist Escalation<br/>48-hour response guarantee]
    D --> E[Step 5: Outcome Validation<br/>≥15% yield improvement or next season free]
    E --> F[Step 6: Renewal & Referral Loop<br/>Word-of-mouth growth]
    F --> A
```
...
```mermaid
graph TD
    subgraph "Option 1: Deepen Advisory Moat"
        A1[IoT hardware partnerships]
        A2[Satellite-based monitoring]
        A3[24-hour response guarantee]
        A4[Voice-based interface]
    end
    
    subgraph "Option 2: Build Output Market Linkage"
        B1[Partner with commodity traders]
        B2[Launch high-value crop marketplace]
        B3[Deduct commission from output sale]
    end
    ...
    subgraph "Option 4: Platform Play"
        D1[Open APIs for third parties]
        D2[Transaction fee model]
        D3[Focus on B2B partnerships]
    end
```
